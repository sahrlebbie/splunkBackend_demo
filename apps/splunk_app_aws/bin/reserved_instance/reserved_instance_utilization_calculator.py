...
import copy
from . import reserved_instance_const as const
from . import reserved_instance_helper as helper
...
class RIUtilizationCalculator(object):
    ...
    def parse_ri_info(self, total_hours, ri_info_search_result):
        ...
        """
        if (not isinstance(ri_info_search_result, six.string_types)) or len(ri_info_search_result.strip(' ')) <= 0:
            return
    ...
    def parse_instance_info(self, instance_info_search_result):
        ...
        if not self.instance_info_valid:
            ...
            return
        self.instance_info = {}
        if (not isinstance(instance_info_search_result, six.string_types)) or len(instance_info_search_result.strip(' ')) <= 0:
            return
    ...
    def apply_ri_with_az_scope(self, hourly_ri_info, hourly_instance_info):
        ...
        """
        for placement, hourly_ri_info_by_placement in six.iteritems(hourly_ri_info):
            if placement == 'null':
                # regional ri
                continue

            for instance_type, ri_hours in six.iteritems(hourly_ri_info_by_placement):
                [family, size] = instance_type.split('.')
    ...
    def apply_ri_with_region_scope(self, hourly_ri_info, hourly_instance_info):
        ...
        if ri_placement in hourly_ri_info:
            for instance_type in list(hourly_ri_info[ri_placement].keys()):
                [family, size] = instance_type.split('.')
                for instance_placement, hourly_instance_info_by_placement in six.iteritems(hourly_instance_info):
        ...
        for instance_placement, hourly_instance_info_by_placement in six.iteritems(hourly_instance_info):
            for instance_type, remained_instance_hours in six.iteritems(hourly_instance_info_by_placement):
    ...
    def apply_ri_with_size_flexibility(self, remained_ri_info, remained_instance_info):
        ...
        if len(list(remained_ri_info.keys())) == 0 or len(list(remained_instance_info.keys())) == 0:
            return

        remained_instance_unit = 0
        for instance_size, remained_instance_hours in six.iteritems(remained_instance_info):
            remained_instance_unit += const.NORMAL_FACTOR[instance_size] * remained_instance_hours

        remained_ri_unit = 0
        for ri_size, remained_ri_hours in six.iteritems(remained_ri_info):
    ...
    def cal_utilization(self):
        if not self.instance_info_valid:
            ...
            return
        for time, hourly_instance_info in six.iteritems(self.instance_info):
