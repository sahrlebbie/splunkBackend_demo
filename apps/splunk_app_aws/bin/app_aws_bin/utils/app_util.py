...
def get_logger(level=logging.INFO):
    ...
    # As of 6.0.0 release, we have detached this module, refer AWSAPP-2166
...
def flatten_args(caller_args, separator=','):
    ...
    for k in caller_args:
        if isinstance(caller_args[k], list):
            v = [x for x in caller_args[k] if x is not None]
            if len(v) > 0:
                args[k] = separator.join(caller_args[k])
    return args


def get_option_from_conf(session_key, conf_name, stanza_name, option_name, app_name=APP_NAME):
