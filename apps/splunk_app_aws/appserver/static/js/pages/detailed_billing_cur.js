...
], function(_, mvc, OneTimePaymentsView, TokenForwarder, MonthSelectorCur, SearchUtil, index) {
    'use strict';
        ...
        if (regions) {
            ...
            if (regions.length === 1 && regions[0] === "*") {
                ...
                let regionsToken = regions
                    .map(region => `detailed_billing_cur.AvailabilityZone="${region}*"`)
                    .join(' OR ');
    ...
    new TokenForwarder(['$productName$'], '$productNameDetailedBilling$', function(productName) {
        if (productName) {
            if (productName === "*") {
                ...
                return `AND detailed_billing_cur.ProductName="${productName}"`;
            }
        } else {
            return TokenForwarder.NO_CHANGE;
        }
    ...
    new TokenForwarder(['$operation$'], '$operationDetailedBilling$', function(operation) {
        if (operation) {
            if (operation === "*") {
                ...
                return `AND detailed_billing_cur.Operation="${operation}"`;
            }
        } else {
            return TokenForwarder.NO_CHANGE;
        }
        ...
        resultsModel.once('data', () => {
            ...
            if (results && results.length > 0) {
                recentResult = results
                    .map(item => 'detailed_billing_cur.InvoiceId=' + item['detailed_billing_cur.InvoiceId'])
                    .join(' OR ');
