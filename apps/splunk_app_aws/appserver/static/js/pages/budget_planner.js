define([
...
], function($, _, mvc, MonthSelector, TokenForwarder, SearchManager, TableView, MessageView, SearchUtil, index) {
    'use strict';
    ...
    function submitBudget(from, to, budget) {
        ...
        if (source === 'cloudwatch') {
            ...
            monthlyCostSPL = `
                | tstats \`aws-data-model-acceleration\` sum(detailed_billing.BlendedCost) as cost FROM datamodel=Detailed_Billing where detailed_billing.LinkedAccountId=${billingAccountId} by _time span=1mon
        ...
        budgetMgr.on('search:done', () => {
            ...
            resultsModel.once('data', function() {
                ...
                if (result && result.length > 0) {
                    ...
                    monthSelector._updateToSelection();
                    monthSelector._updateTimerange();
