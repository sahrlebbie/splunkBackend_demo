...
define([
    ...
    return Backbone.View.extend({
        ...
        _getTimeLineDetails: function(event) {
            var includeList = TimelineConstants.CHANGE_PROPERTIES_INCLUDE_LIST[event.service],
                detailsHTML = '';

            if (includeList) {
                for (var attribute in includeList) {
                    if (!_.isUndefined(event.changeProperties[attribute])) {
                        var propertyText = (includeList[attribute].makeText ? includeList[attribute].makeText(event.changeProperties[attribute]) : event.changeProperties[attribute]);
                        detailsHTML += `<tr><th>${includeList[attribute].label}:</th><td>${propertyText}</td></tr>`;
                    }
                }
            }
