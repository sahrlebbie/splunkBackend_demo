define([
    ...
    'app/utils/HelpLinks',
    'app/libs/canvg'
], (_, $, Backbone, Utils, mvc, SimpleSplunkView, d3, SearchManager, TopologyConfig, TopologyAlgorithm, MessageView, PlayBackView, KeyFrameUtil, index, SearchUtil, HelpLinks) => {
            ...
            this.template = `\
                <div class="chart-title">\
                    <div class="recommZone disabled"> \
                        <div id="recommBtn" style="display:none;" data-toggle="tooltip" data-placement="left" data-container="body" data-content="Vivamus sagittis lacus vel augue laoreet rutrum faucibus." title="Insights"></div>\
                ...
                <div id="recomm-dependency"> \
                    ...
                    <p>The Insights Service depends on the Python for Scientific Computing application, which is not installed or incorrectly configured. <%=learnmore%></p> \
                </div> \
            `;
            this.svgContext = {
                svg: null,
        ...
        createView() {
            ...
            SearchUtil.search('|inputlookup instance_type', {
                ...
                if (resultData.length != 0 && resultData[0]['value'] == 'on-prem') {
					$('#recommBtn').css({"display": "block"});
					 if (this.model.viewModel.get('hasMLLib') === false){
						this._disableRecommZone();
					}
				}
            }, function(err) {
                console.log("Error while disabling the Recommend zone: " + err);
            });
        ...
        _generateKPIs() {
            ...
            $.get(billing_type_url + "?output_mode=json").done(function(data) {
                ...
                if(billing_type[0] !== undefined && billing_type[0] == "Billing_CUR")
                    KPIS.billing.search = "|inputlookup topology_billing_metrics_cur | table billing, name";
