...
define([
    ...
    const LOOKUP_MAP = {
        ...
        'Billing (Legacy)': "| `aws-billing-datamodel-tags`",
        'Billing (Cost and Usage Report)': "| `aws-billing-datamodel-tags-cur`"
    };
    ...
    let TagInputView = BaseView.extend({
        ...
        onKeydown(e) {
            if (e.keyCode === 38 || e.keyCode === 40) {
                ...
                if ($currentActive.length === 0 || !this.$('.tag-autocomplete').is(':visible')) {
                    ...
                    if (_tokenModel && submitModel) {
                        submitModel.set(_tokenModel.toJSON());
                      }
                } else {
        ...
        _showAutoComplete(items) {
            let html = items.map(item => {
                let text = _.escape(item.text);
                let stage = _.escape(item.stage);
                return `<li data-stage="${stage}">${text}</li>`;
            }).join('');
        ...
        _fetchValues(key, value) {
            var value_spl = ""
            var cur_tag_macro = `\`aws-billing-cur-tag-value(${key})\``
            var legacy_tag_macro = `\`aws-billing-tag-value(${key})\``
            if (_spl.indexOf("`aws-billing-datamodel-tags`") > -1){
                value_spl = `${_spl} | search key=CASE(${key}) | ${legacy_tag_macro} | search value=* | fields value`
            }
            else if(_spl.indexOf("`aws-billing-datamodel-tags-cur`") > -1){
                value_spl = `${_spl} | search key=CASE(${key}) | ${cur_tag_macro} | search value=* | fields value`
            }
            else{
                value_spl = `${_spl} | search key=CASE(${key}) | dedup value | fields value`
            }
            this._showLoading();
            this._cachedKey = key;
            SearchUtil.search(value_spl).then((lookupData) => {
            ...
            let fullSPL = inputs
                ...
                .sort((a, b) => {
                    ...
                    if (a.indexOf('`aws-billing-datamodel-tags`') > -1) {
                        return -1;
                    } else if (a.indexOf('`aws-billing-datamodel-tags-cur`') > -1) {
                        return -1;
                    } else if (b.indexOf('`aws-billing-datamodel-tags`') > -1) {
                        return 1;
                    } else if (b.indexOf('`aws-billing-datamodel-tags-cur`') > -1) {
                        return 1;
                    }
