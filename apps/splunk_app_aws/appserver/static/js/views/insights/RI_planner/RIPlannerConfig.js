define([
    ...
    return {
        ...
        PARAM:
            '| tstats `aws-data-model-acceleration` count FROM datamodel=Instance_Hour where instance_hour.AvailabilityZone!="Unspecified" ' +
        ...
        RI:
            '| tstats `aws-data-model-acceleration` count as <%=name %> FROM datamodel=Instance_Hour where <%= dedupSpl %> AND instance_hour.LinkedAccountId="<%= accountId %>" ' +
            ...
            '| fillnull',
        SIZE_FLEXIBILITY_RI:
            '| tstats `aws-data-model-acceleration` count FROM datamodel=Instance_Hour where <%= dedupSpl %> AND instance_hour.LinkedAccountId="<%= accountId %>"' +
            ' AND instance_hour.AvailabilityZone="<%= location %>" AND instance_hour.instance_type="<%= type %>" ' +
