...
], function ($, _, Backbone, mvc, TokenHelper, SearchManager, SearchUtil, TokenForwarder, SearchQueueManager, Helper, Config, TableConfig, BillingCurUtil, index, RIPlannerExistingRITemplate, RIPlannerOptimalRITemplate) {
    'use strict';
    const moment = index.moment;
    const utils = index.utils;
    const DATE_FORMAT = 'YYYYMMDD-';

    class RIPlannerControllerCur {
        constructor(model) {
            this.model = model;
            this.cache = {};
            ...
            SearchUtil.search(Config.BILLING_REPORT_ENDTIME, {
                ...
                results.forEach((row) => {
                    ...
                    if (timestr >= lastYearStartTime) {
                        s3keyByMonth[timestr] = timestr;
                        assemblyId[timestr] = row['assemblyId'];
                    }

                });
                this.billingReportDedupSPL = BillingCurUtil.getDedupSpl(s3keyByMonth,assemblyId, Config.DEDUP_PREFIX);
                dfd.resolve();
            }));
            return dfd;
        }
        ...
        _assembleRow(id) {
            ...
            if (ri === 'N/A') {
                ...
                detail.link = 'RI_planner_detail_cur?' + Object.keys(linkParams).map(key => `${key}=${linkParams[key]}`).join('&');
            }
            if (existingRIs === 'N/A') {
