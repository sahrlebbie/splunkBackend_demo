define([
    ...
    return {
        ...
        DEDUP_PREFIX: 'instance_hour_cur.',
        EVENT: {
            DRAG_POINT: 'point',
            SELECT_TIME: 'time',
            RESET: 'reset'
        ...
        PARAM:
            '| tstats `aws-data-model-acceleration` count FROM datamodel=Instance_Hour_CUR where instance_hour_cur.InvoiceId=* instance_hour_cur.AvailabilityZone!="Unspecified" ' +
            ' by instance_hour_cur.LinkedAccountId instance_hour_cur.instance_type instance_hour_cur.platform instance_hour_cur.tenancy instance_hour_cur.AvailabilityZone ' +
            '| rename instance_hour_cur.AvailabilityZone as availability_zone, instance_hour_cur.LinkedAccountId as aws_account_id, ' +
            ' instance_hour_cur.instance_type as instance_type, instance_hour_cur.platform as platform, instance_hour_cur.tenancy as tenancy ' +
        ...
        RI:
            '| tstats `aws-data-model-acceleration` sum(instance_hour_cur.UsageAmount) as count FROM datamodel=Instance_Hour_CUR where instance_hour_cur.InvoiceId=* <%= dedupSpl %> AND instance_hour_cur.LinkedAccountId="<%= accountId %>" ' +
            ' AND instance_hour_cur.AvailabilityZone="<%= location %>" AND instance_hour_cur.instance_type="<%= type %>" ' +
            ' AND instance_hour_cur.platform="<%= platform %>" AND instance_hour_cur.tenancy="<%= tenancy %>" by instance_hour_cur.instance_type, _time span=1d ' +
            '| curdailytohourly' +
            '| timechart span=1<%=slice %> sum(count) as <%=name %> ' +
            '| fillnull',
        SIZE_FLEXIBILITY_RI:
            '| tstats `aws-data-model-acceleration` sum(instance_hour_cur.UsageAmount) as count FROM datamodel=Instance_Hour_CUR where instance_hour_cur.InvoiceId=* <%= dedupSpl %> AND instance_hour_cur.LinkedAccountId="<%= accountId %>"' +
            ' AND instance_hour_cur.AvailabilityZone="<%= location %>" AND instance_hour_cur.instance_type="<%= type %>" ' +
            ' AND instance_hour_cur.platform="<%= platform %>" AND instance_hour_cur.tenancy="<%= tenancy %>" by instance_hour_cur.instance_type, _time span=1d ' +
            '| curdailytohourly ' +
            '| rename instance_hour_cur.instance_type as instance_type ' +
        ...
        BILLING_REPORT_ENDTIME:
            '| inputlookup billing_report_assemblyid_cur ' +
            '| sort 0 - timestr',
        PREDICT_RI:
