...
], function ($, ChartHelper, RIPlannerHelperCur, Config, index) {
            ...
            if (originalEvent.target.getAttribute('class') === null) { //path
                if (hoverPoint) {
                    ...
                    if (options.drag && hoverPoint.drag !== false) {
                        ...
                        helper.dragStart(RIPlannerHelperCur.timeToIndex(dragPoint.x), dragPoint.y);
        ...
        function drop(e) {
            if (dragPoint) {
                ...
                if (isClick && dragPoint.options.selected) { // double click to not choose
                    ...
                    helper.deleteAnchor(RIPlannerHelperCur.timeToIndex(dragPoint.x));
                } else {
                    dragPoint.update({marker: {enabled: true, radius: 6}});
                    dragPoint.options.selected = true;
                    if (isClick) { // double click to choose
                        helper.addAnchor(RIPlannerHelperCur.timeToIndex(dragPoint.x));
                    } else { // drag
