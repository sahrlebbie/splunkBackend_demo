define(
    ...
    function (_, Backbone, index) {
        const SplunkDBase = index.SplunkDBase;
        ...
        var syncCreate = function (model, options) {
            ...
            $.extend(true, defaults.data, model.includeListAttributes());
        ...
        var syncUpdate = function (model, options) {
            ...
            $.extend(true, defaults.data, model.includeListAttributes(fetchOptions));
            $.extend(true, defaults, options);
        ...
        var syncPatch = function (model, options) {
            ...
            $.extend(true, defaults.data, (options.attrs && model.includeListPassedInAttributes(options.attrs)) || {});
        ...
        return SplunkDBase.extend({
            ...
            includeListAttributes: function (fetchOptions) {
                var includeListOptAndReq = this.getAttributeNames(),
                    includeListWild = this.splunkDWhiteList.get('wildcard') || [],
                    contentAttrs = this.entry.content.filterByKeys(includeListOptAndReq,
                        {allowEmpty: true},
                        fetchOptions);

                return _.extend(contentAttrs, this.entry.content.filterByWildcards(includeListWild, {allowEmpty: true}, fetchOptions));
            },
            fetch: function () {
                var self = this;
