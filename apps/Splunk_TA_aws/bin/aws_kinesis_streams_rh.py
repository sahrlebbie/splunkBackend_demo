...
class KinesisStreams(admin.MConfigHandler):
    ...
    def handleList(self, conf_info):
        ...
        if int(self.callerArgs.get('private_endpoint_enabled',[0])[0]):
            endpoint_urls = {
                "kinesis_private_endpoint_url" : self.callerArgs.get('kinesis_private_endpoint_url')[0],
                "sts_private_endpoint_url" : self.callerArgs.get('sts_private_endpoint_url')[0],
            }
            if endpoint_urls["kinesis_private_endpoint_url"] and endpoint_urls["sts_private_endpoint_url"]:
            ...
            kinesis_endpoint_url = "https://{}.{}.amazonaws.com/".format("kinesis", self.callerArgs[tac.region][0])
        ...
        cred = cred_service.load(aws_account, aws_iam_role, self.callerArgs[tac.region][0], endpoint_url=sts_endpoint_url)
