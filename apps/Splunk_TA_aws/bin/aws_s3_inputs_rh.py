...
class GenericS3Handler(AdminExternalHandler):
        ...
        if isd and isd != 'default' and tsd:
            try:
                ...
                raise RestError(
                    ...
                    'Wrong datetime format: {}'.format(str(ex).capitalize())
                )

            if tsd < isd:
                raise RestError(
                    BAD_REQUEST_STATUS_CODE, 
                    'Invalid datetime range. "{}" should be greater than "{}"'.format(
                        END_DATETIME_FIELD_NAME,
                        START_DATETIME_FIELD_NAME
                    )
                )
            elif tsd == isd:
                raise RestError(
                    BAD_REQUEST_STATUS_CODE,
                    'Invalid datetime range. "{}" and "{}" can\'t be same'.format(
                        END_DATETIME_FIELD_NAME,
                        START_DATETIME_FIELD_NAME
                    )
                )

        super(GenericS3Handler, self).handleCreate(confInfo)
    ...
    def get(self, field_name):
        field = self.callerArgs.data.get(field_name)
