...
STS_PRIVATE_ENDPOINT_PATTERN = PRIVATE_ENDPOINT_PATTERN.replace('<service_name>', 'sts')
...
class AccountRestHandler(admin.MConfigHandler):
    ...
    def _getAccountConfig(self, key, exts):
        ...
        if temp:
            ...
            if result['account_id'] and result['name']:
                account_util.append_account_to_summary(name=result['name'],
    ...
    def handleCreate(self, confInfo):
        try:
            private_endpoint_enabled = int(self.callerArgs.data['private_endpoint_enabled'][0])
            sts_private_endpoint_url = self.callerArgs.data['sts_private_endpoint_url'][0]
            sts_private_endpoint_url = None if not sts_private_endpoint_url or not sts_private_endpoint_url.strip() else sts_private_endpoint_url
            if private_endpoint_enabled: 
                if not sts_private_endpoint_url:
                    raise Exception("Field Private Endpoint URL (STS) is required")
                else:
                    if not re.match(STS_PRIVATE_ENDPOINT_PATTERN,sts_private_endpoint_url):
            ...
            cm.create_stanza(CONF_FOR_ACCOUNT_EXT_FIELDS, self.callerArgs.id, {"category": cate,"sts_region":reg,"private_endpoint_enabled":use_cust_ep,"sts_private_endpoint_url":cust_ep,})
    ...
    def handleEdit(self, confInfo):
        try:
            ...
            try:
                ...
                pass
            endpoint_url = sts_private_endpoint_url if private_endpoint_enabled else None
            ...
            if cm.stanza_exist(CONF_FOR_ACCOUNT_EXT_FIELDS, account_name):
                cm.update_stanza(CONF_FOR_ACCOUNT_EXT_FIELDS, account_name, {'category': category,"sts_region":region,"private_endpoint_enabled":private_endpoint_enabled,"sts_private_endpoint_url":sts_private_endpoint_url})
            else:
                cm.create_stanza(CONF_FOR_ACCOUNT_EXT_FIELDS, account_name, {'category': category,"sts_region":region,"private_endpoint_enabled":private_endpoint_enabled,"sts_private_endpoint_url":sts_private_endpoint_url})
