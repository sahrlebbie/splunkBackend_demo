...
class AWSCredentialsService(object):
    ...
    def load(self, aws_account_name, aws_iam_role_name=None, region_name=None, endpoint_url=None):
        logger.info(
            'begin loading credentials',
            aws_account=aws_account_name,
            aws_iam_role=aws_iam_role_name
        )
        credentials = self._load(aws_account_name, aws_iam_role_name, region_name, endpoint_url)
    ...
    def _load(self, aws_account_name, aws_iam_role_name=None, region_name=None, endpoint_url=None):
    ...
    def _load_source_credentials(cls, account, region_name, endpoint_url=None):
        ...
        sts = cls.create_sts_client(key_id, secret_key, token, region_name, endpoint_url)
    ...
    @classmethod
    def create_sts_client(cls, key_id, secret_key, token, region_name, endpoint_url=None):
        if not endpoint_url:
            endpoint_url = "https://{}.{}.amazonaws.com/".format("sts",region_name)
        return boto3.client(
            'sts',
