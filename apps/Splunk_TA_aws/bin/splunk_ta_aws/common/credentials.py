...
class AWSRawCredentials(object):
    ...
    def client(self, service_name, region_name, boto_session=None, endpoint_url=None):
        ...
        ''' endpoint_url will be used for connecting to specific service for API call.
         endpoint_url can be either a valid vpce endpoint or a standard regional endpoint '''
        if not endpoint_url:
            if service_name not in ['cloudwatch','elb','elbv2']:
                params['endpoint_url'] = "https://{}.{}.amazonaws.com/".format(
...
class AWSAccountProvider(AWSCredentialsProvider):
    ...
    def load(self):
        ...
        """
        return self._aws_account.load_credentials(region_name=self.region_name, endpoint_url=self.endpoint_url)
...
class AWSCredentialsCache(object):
    ...
    def client(self, service_name, region_name, boto_session=None, endpoint_url=None):
        return self._credentials.client(service_name, region_name, boto_session, endpoint_url)
