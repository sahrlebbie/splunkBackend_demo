...
def validate_keys(session_key, **account_info):
    ...
    try:
        ...
        err_msg = 'Failed to validate account due to %s' % e.response['Error']['Code']
        if 'Credential should be scoped to a valid region' in e.response['Error'].get('Message'):
...
def _get_caller_identity(account):
    ...
    if account.get('endpoint_url'):
        ...
        credentials['endpoint_url'] = "https://sts.{}.amazonaws.com/".format(credentials['region_name'])
