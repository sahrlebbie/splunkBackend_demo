...
class S3NoticeParser(object):
    ...
    def _make(cls, record):
        ...
        key = urlparse.unquote(s3object['key'])
...
class CrowdstrikeNoticeParser(object):
    """
    ...
    def parse(self):
        ...
        for _file in files:
            if(self._s3_agent.checkSuccessFile(bucket, region, prefix)):
                s3_list.append(self._make(region, bucket, _file))
            else:
                logger.debug("Skipping the message as _SUCCESS file is not generated in s3 bucket")
                continue
        return s3_list
...
class SQSBasedS3PipelineAdapter(object):
    ...
    def __init__(self, app, config, credentials, sqs_agent, s3_agent, s3_region_cache,
                 using_dlq, decode, event_writer, max_receive_count, exit_on_idle, temp_folder):
        self._app = app
        self._config = config
    ...
    def discover(self):
        ...
        if (not self.using_dlq) and (not attributes.redrive_policy):
    ...
    def _open_temp_file(self):
        try:
            ...
            stf = tempfile.SpooledTemporaryFile(max_size=max_size, dir=folder)
...
class SQSAgent(object):
    def __init__(self, url, region, credentials, endpoint_url=None):
        self._queue = SQSQueue(url, region, endpoint_url)
        self._batch_size = 0
        self._credentials = credentials
...
class S3Agent(object):
    ...
    def checkSuccessFile(self, bucket_name, region, prefix, session=None):
        try:
            bucket = S3Bucket(bucket_name, region)
            s3 = bucket.client(self._credentials, session)
            keys = s3.list_objects(Bucket=bucket_name, Prefix=prefix)
            if(k['Key'] == prefix+"_SUCCESS" for k in keys['Contents']):
                return True
            return False
        except Exception as e:
...
class SQSBasedS3DataInput(object):
    ...
    def create_credentials(self, config):
        ...
        factory = AWSCredentialsProviderFactory(config, self._args.get("sqs_queue_region"), sts_endpoint_url)
    ...
    def create_sqs_agent(self, credential):
        ...
        args = self._extract_arguments(parser)
        sqs_endpoint_url = tacommon.get_endpoint_url(self._args, "sqs_private_endpoint_url")
        agent = SQSAgent(args.sqs_queue_url, args.sqs_queue_region, credential, sqs_endpoint_url)
