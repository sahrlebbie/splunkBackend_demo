...
class CheckPointer(object):
    ...
    def __init__(self, checkpoint_dir, stanza_name, bucket_name,
        ...
        stanza_hexdigest = pymd5.md5("{}_{}".format(stanza_name, bucket_name).encode('utf-8')).hexdigest()
    ...
    def get_oldest_item(self):
        ...
        oi = sorted(six.iteritems(self._data['items']), key=lambda x: x[1][1])[0]
...
class MyScript(smi.Script):
    ...
    def get_scheme(self):
        ...
        scheme.add_argument(smi.Argument("aws_s3_region", title="AWS Region",
    ...
    def validate_input(self, definition):
        ...
        try:
            ...
            logger.error("get exception while validating input: " + str(e))
                ...
                try:
                    ...
                    if os.name == 'nt':
                        ...
                        try:
                            ...
                            logger.warning("fail to open the reader, use original file handler. reason:" + str(e))
                            reader = csv.reader(opened_file,
                                                skipinitialspace=True, delimiter=',', quotechar='"')
                    ...
                    for row in reader:
                        ...
                        try:
                            ...
                            logger.error("there's error to handle this row: " + str(e)
    ...
    def _stream_monthly_billing_item(self, key, checkpoint_item, checkpointer):
        ...
        try:
            ...
            try:
                ...
                logger.error("catch exception while parsing global time str: " + str(ex) +
            ...
            for row in reader:
                ...
                md5 = pymd5.md5(str(row).encode('utf-8') + str(key.etag).encode('utf-8')).hexdigest()
                ...
                try:
                    ...
                    logger.error("there's error to handle this row: " + str(e) + ", record its checksum as handled.")
                    logger.error(traceback.format_exc())
                else:
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        for open_file in self.file_open_list:
            try:
                ...
                logger.error("fail to delete file: file name=" + str(open_file) + " error=" + str(_e)
        ...
        for file_name in self.file_list:
            try:
                ...
                logger.error("fail to delete file: file name=" + file_name + " error=" + str(_e)
                             + str(exc_type) + str(exc_val) + str(exc_tb))
        ...
        for folder_name in self.folder_list:
            try:
                ...
                logger.error("fail to delete file: folder name=" + folder_name + " error=" + str(_e)
                             + str(exc_type) + str(exc_val) + str(exc_tb))
