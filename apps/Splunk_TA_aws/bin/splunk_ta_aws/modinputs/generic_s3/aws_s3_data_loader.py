...
class S3DataLoader(object):
    ...
    def _get_s3_client(self, credentials):
        ...
        default_s3_url = "https://{}.{}.amazonaws.com".format("s3", region)
        s3_endpoint_url = tacommon.get_endpoint_url(self._config, "s3_private_endpoint_url", default_s3_url)
        return boto3.client(
            's3',
            region_name=region,
    ...
    def _discover_keys(self, index_store):
        ...
        def _refresh_creds():
            self.credentials = self._check_and_refresh_bucket(self.credentials, self.s3_conn)
        ...
        self.s3_conn = self._get_s3_client(self.credentials)
        last_modified = self._normalize_last_modified(index_store.last_modified())
        keys = s3common.get_keys(_refresh_creds,
            self.s3_conn, self._config[asc.bucket_name], self._config.get(asc.prefix, ""),
            self._config.get(asc.whitelist), self._config.get(asc.blacklist), last_modified)
    ...
    def _fetch_key(self, s3_conn, key_name, loader_service):
        try:
            ...
            skp.increase_error_count(
                key_store, self._config[asc.max_retries], key, logger, self._config[asc.bucket_name],
    ...
    def _check_and_refresh_bucket(self, credentials, client):
        ...
        client._request_signer._credentials = botocore.credentials.Credentials(
    ...
    def _generate_credentials(self):
        sts_endpoint_url = tacommon.get_endpoint_url(self._config, "sts_private_endpoint_url")
