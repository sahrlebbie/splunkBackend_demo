...
class S3CkptPool(object):
    ...
    class S3CkptItem(object):
        ...
        def __init__(self, ckpt_name, ckpt_dir):
            ckpt_file_idx, ckpt_file_key = list(map(
                ckpt_file,
...
class S3IndexCheckpointer(object):
    ...
    def _get_meta_fields(self, new):
        meta_fields = None
        try:
            meta_fields = _unify_type_of_fields(
                self._idx_ckpt.get(asc.meta_fields)
            )
    ...
    def get(self, key_name):
        return _unify_type_of_fields(
            self._idx_ckpt.get(key_name)
        )
...
class S3KeyCheckpointer(object):
    ...
    def _pop_ckpt_item(self, key):
        try:
            key_item = _unify_type_of_fields(
                self._key_ckpt.get(self._key_name)
            )
        except KeyError:
            logger.debug(
