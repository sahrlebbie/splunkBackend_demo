...
def increase_error_count(key_store, max_retries, key, logger, bucket_name, count=1):
...
class S3KeyProcesser(object):
    ...
    def __init__(self, s3_conn, loader_service, key_object, config, logger):
        ...
        with logging.LogContext(datainput=self._config[asc.data_input],
            ...
            try:
                ...
                increase_error_count(
                    ...
                    self._key, self._logger, self._config[asc.bucket_name]
                )
...
class S3KeyCloudTrailProcesser(S3KeyProcesser):
    ...
    def __init__(self, s3_conn, loader_service, s3_key_object, config, logger):
        super(S3KeyCloudTrailProcesser,  self).__init__(
            s3_conn, loader_service, s3_key_object, config, logger)
