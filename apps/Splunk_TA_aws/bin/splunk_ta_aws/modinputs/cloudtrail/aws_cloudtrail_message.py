...
class S3ConnectionPool(object):
    ...
    @classmethod
    def get_conn(cls, key_id, secret_key, bucket, default_region='us-east-1', s3_endpoint_url=None):
        """Return s3 connection to region where bucket is located."""
        bucket_region = cls.get_bucket_region(bucket, key_id, secret_key, default_region, s3_endpoint_url)
        if not s3_endpoint_url:
            s3_endpoint_url = "https://{}.{}.amazonaws.com/".format('s3',bucket_region)
    ...
    @classmethod
    def get_bucket_region(cls, bucket, key_id, secret_key, default_region='us-east-1', s3_endpoint_url=None):
        if not s3_endpoint_url:
            s3_enpoint_url = "https://{}.{}.amazonaws.com/".format('s3',default_region)
...
class MessageProcessor(six.with_metaclass(ctcommon.ThreadLocalSingleton, object)):
    ...
    def run(self, session_key, datainput, aws_account, message_id, message,
            ...
            sourcetype, index, default_region="us-east-1", s3_endpoint_url=None):
        """
        Process Message.
        ...
        for bucket_name, key_name in self._s3_keys():
            ...
            try:
                res = self._process_s3_key(bucket_name, key_name, default_region, s3_endpoint_url)
            except Exception:
    ...
    def _load_s3_key(self, bucket_name, key_name, default_region='us-east-1', s3_endpoint_url=None):
        s3_key_cont = {}
        s3_conn = s3_key = None

        try:
            s3_conn = S3ConnectionPool.get_conn(
                self.aws_account['key_id'], self.aws_account['secret_key'], bucket_name, default_region, s3_endpoint_url)
            s3_key = s3_conn.get_object(Bucket=bucket_name, Key=key_name)
        except ClientError as ex:
            logger.error('Get S3 Key Failed', datainput=self.datainput,
    ...
    def _process_s3_key(self, bucket_name, key_name, default_region="us-east-1", s3_endpoint_url=None):
        logger.debug('Start getting S3 key', datainput=self.datainput,
                     s3_bucket_name=bucket_name, s3_key_name=key_name)

        s3_key_cont = self._load_s3_key(bucket_name, key_name, default_region, s3_endpoint_url)
        stats = {
